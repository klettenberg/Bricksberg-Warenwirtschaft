...
function lww_execute_ai_demand_analysis($post_id, $endpoint_slug, $prompt) {
    ...
    if (is_wp_error($response)) {
        lww_log_api_call($ai_provider, $endpoint_slug, false, 0, ['post_id' => $post_id, 'error' => $response->get_error_message()], wp_json_encode($response));
        return $response;
    }
    ...
    if ($response_code !== 200) {
        ...
        lww_log_api_call($ai_provider, $endpoint_slug, false, 0, ['post_id' => $post_id, 'error' => $error_message, 'response_code' => $response_code], $response_body);
    ...
    lww_log_api_call($ai_provider, $endpoint_slug, true, $simulated_cost, ['post_id' => $post_id, 'score' => $score], $response_body);
